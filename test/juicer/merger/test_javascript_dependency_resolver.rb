...
class TestJavaScriptDependencyResolver < Test::Unit::TestCase
  ...
  def test_resolve
    b_file = File.expand_path(File.join($DATA_DIR, 'b.js'))
    a_file = File.expand_path(File.join($DATA_DIR, 'a.js'))

    files = @resolver.resolve(File.join($DATA_DIR, 'a.js')) do |file|
    ...
    files = @resolver.resolve(File.join($DATA_DIR, 'b.js')) do |file|
      assert b_file == file || a_file == file
      true
    end
