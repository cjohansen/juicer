...
class TestYuiCompressorInstall < Test::Unit::TestCase
  ...
  def test_check_installation
    assert_false Juicer::Install::YuiCompressorInstaller.installed?(path("juicer_home"))
  end
  ...
  def test_install_should_download_jar_and_readme
    ...
    assert File.exists?(path("juicer_home/lib/yui/bin"))
    assert_match /yuicompressor-\d\.\d\.\d\.jar/, Dir.glob(path("juicer_home/lib/yui/bin/*"))[0]

    files = Dir.glob(path("juicer_home/lib/yui/*.*.*/**/*"))
  ...
  def test_uninstall_should_remove_all_files_and_empty_directories
    Juicer::Install::YuiCompressorInstaller.install(path("juicer_home"))
    assert File.exists?(path("juicer_home/lib/yui/bin"))
    assert_equal 3, Dir.glob(path("juicer_home/lib/yui/**/*")).find_all { |f| File.file?(f) }.length
  ...
  def test_install_specific_version
    ...
    assert File.exists?(path("juicer_home/lib/yui/bin"))
    assert File.exists?(path("juicer_home/lib/yui/2.3.5"))
    assert_equal "yuicompressor-2.3.5.jar", Dir.glob(path("juicer_home/lib/yui/bin/*"))[0]
  end
  ...
  def test_uninstall_should_leave_directories_when_other_versions_are_installed
    ...
    Juicer::Install::YuiCompressorInstaller.install(path("juicer_home"), "2.3.5")
    assert File.exists?(path("juicer_home/lib/yui/bin"))
    assert_equal 6, Dir.glob(path("juicer_home/lib/yui/**/*")).find_all { |f| File.file?(f) }.length
