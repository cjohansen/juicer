...
class TestYuiCompressorInstall < Test::Unit::TestCase
  def setup
    @installer = Juicer::Install::YuiCompressorInstaller.new(path("juicer_home"))
  end

  def teardown
    FileUtils.rm_rf(path("juicer_home/lib")) if File.exists?(path("juicer_home/lib"))
  end
  ...
  def test_install_specific_version
    ...
    assert_equal "yuicompressor-2.3.5.jar", File.basename(Dir.glob(path("juicer_home/lib/yui_compressor/bin/*"))[0])
  end
